...
export default function ActivityCard({ activity, onToggle, onEdit, onDelete }: ActivityCardProps) {
  ...
  return (
    <div
      className={`bg-white/5 border border-white/10 rounded-2xl p-4 flex items-start gap-3 transition-all duration-200 ${
        isDone ? "opacity-60" : "hover:bg-white/[0.07]"
      }`}
    >
      {/* Checkbox */}
      <button
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium leading-snug ${isDone ? "line-through text-primary-500" : "text-white"}`}>
          {activity.title}
        </p>
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          {/* Badge da matéria */}
          {activity.subject && (
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium ${colors.bg} ${colors.text}`}>
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        <button
          ...
          className="p-1.5 rounded-lg hover:bg-white/10 text-primary-500 hover:text-primary-300 transition-colors cursor-pointer"
          title="Editar"
        >
        ...
        <button
          ...
          className="p-1.5 rounded-lg hover:bg-red-500/15 text-primary-500 hover:text-red-400 transition-colors cursor-pointer"
          title="Excluir"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5">
